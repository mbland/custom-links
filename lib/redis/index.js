...
RedisClient.prototype.fetchShortLinksFromTargetLinks = function(targetKeys) {
  return Promise.all(targetKeys.map(targetKey =>
    this.getShortLinksFromTargetLink(targetKey)
  )).then(shortLinks =>
    targetKeys.reduce((results, targetLinkKey, i) => {
      results[Keys.getTargetLinkFromKey(targetLinkKey)] = shortLinks[i]
      return results
    }, {})
  )
}
...
RedisClient.prototype.searchTargetLinks = function(searchString) {
  ...
  return searchHelper.scan().then(links =>
    this.fetchShortLinksFromTargetLinks(links))
}
