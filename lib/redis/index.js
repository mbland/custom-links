...
class AutocompleteIndexer {
  ...
  removeLink(link) {
    return PromiseHelper.do(done => {
      ...
      this.impl.zrem(SEARCH_LINKS_KEY, link.slice(1) + '*', done)
    })
  }
  ...
  getSearchPrefixRank(prefix) {
    return PromiseHelper.do(done => {
      this.impl.zrank(SEARCH_LINKS_KEY, prefix, done)
    })
  }
  ...
  getSearchPrefixRange(rangeSize) {
    return rank => PromiseHelper.do(done => {
      rank === null ? done(null, []) :
        this.impl.zrange(SEARCH_LINKS_KEY, rank, rank + rangeSize - 1, done)
    })
  }
}
...
class TargetIndexer {
  ...
  addLink(link, target) {
    return PromiseHelper.do(done => {
      this.impl.lpush(TargetIndexer.key(target), link, done)
    })
  }
  ...
  getLinksToTarget(target) {
    var getLinks = done => {
      this.impl.lrange(TargetIndexer.key(target), 0, -1, done)
    }
...
function removeItemFromList(clientImpl, key, item) {
  return PromiseHelper.expect(1, done => clientImpl.lrem(key, 1, item, done))
}
