...
RedisClient.prototype.fetchShortLinkFromTargetLink = function(links) {
  return Promise.all(links.map(link => {
    return this.getShortLinkFromTargetLink(link).then(shortLink => {
      return shortLink
    })
  }))
}

RedisClient.prototype.getShortLinkFromTargetLink = function(link) {
  return new Promise((resolve, reject) => {
    this.impl.lrange(link, 0, -1, (err, linkData) => {
      if (err) {
        return reject(err)
      } else if (linkData) {
        resolve(linkData)
...
RedisClient.prototype.getLinks = function(searchString) {
  var searchHelper = new SearchHelper(this.impl, searchString, 'short_link')
...
RedisClient.prototype.searchTargetLinks = function(searchString) {
  var searchHelper = new SearchHelper(this.impl, searchString, 'target_link')
  return searchHelper.scan().then(links =>
    this.fetchShortLinkFromTargetLink(links.sort()))
}
