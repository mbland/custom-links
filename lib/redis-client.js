...
function checkResultEquals(expected, func) {
...
RedisClient.prototype.userExists = function(userId) {
  return checkResultEquals(1, done => this.client.exists(userId, done))
}
...
RedisClient.prototype.findOrCreateUser = function(userId) {
  return this.userExists(userId)
    .then(exists => {
      ...
      return checkResultEquals(1, done => this.client.lpush(userId, '', done))
    })
}
...
RedisClient.prototype.removeUrlFromOwner = function(owner, url) {
  return checkResultEquals(1, done => this.client.lrem(owner, 1, url, done))
}
...
RedisClient.prototype.updateProperty = function(url, property, value) {
  return this.getRedirect(url)
    .then(urlData => {
      ...
      return checkResultEquals(0, done => {
...
RedisClient.prototype.deleteRedirection = function(url) {
  return checkResultEquals(1, done => this.client.del(url, done))
}
