...
RedisClient.prototype.createLink = function(link, location, owner) {
  return new Promise((resolve, reject) => {
    this.client.hsetnx(link, 'owner', owner, (err, result) => {
      ...
      this.client.hmset(link, 'location', location, 'count', 0, err => {
        if (err) {
          return reject(new Error('redirection created for ' + link +
